...
export function ApiClientRiskBadge({ risk }: { risk: ClientRisk }) { return <span className={cn("inline-flex rounded-full border px-2 py-1 font-mono text-xs font-semibold", riskStyles[risk])}>● {risk}</span>; }
export function ApiClientStatusBadge({ status }: { status: ClientStatus }) { return <span className={cn("inline-flex rounded-full px-2 py-1 text-xs font-semibold", statusStyles[status])}>◉ {status}</span>; }
export function EnvironmentBadge({ environment }: { environment: Environment }) { const style = environment === "Production" ? "bg-red-500/10 text-red-600" : environment === "Staging" ? "bg-amber-500/10 text-amber-600" : "bg-emerald-500/10 text-emerald-600"; return <span className={cn("rounded px-1.5 py-0.5 text-xs font-semibold", style)}>{environment}</span>; }
