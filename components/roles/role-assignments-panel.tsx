...
export function RoleAssignmentCard({ role }: { role: Role }) {
  ...
  return <section className="overflow-hidden rounded-xl border bg-card shadow-[var(--dv-shadow)]"><header className={`flex items-center justify-between gap-3 border-b p-4 ${roleColor[role.color].split(" ")[0]}`}><div><h3 className="text-sm font-bold">{role.name}</h3><p className="mt-1 text-xs text-muted-foreground">{role.description}</p></div><span className="text-right"><b className="block text-lg">{users.length}</b><small className="text-xs">users assigned</small></span></header><div className="border-b px-4 py-2 text-xs"><span className="mr-3 font-mono font-semibold uppercase text-muted-foreground">Sites</span>{sites.map((site) => <span key={site} className="mr-1 inline-flex items-center gap-1 rounded-md bg-muted px-2 py-1"><Building2 className="size-2.5" />{site}</span>)}<span className="ml-4 mr-2 font-mono font-semibold uppercase text-muted-foreground">Departments</span>{departments.map((department) => <span key={department} className="mr-1 rounded-md bg-muted px-2 py-1">{department}</span>)}</div><div>{users.map((user) => <div key={user.id} className="flex items-center gap-3 border-b px-4 py-3 last:border-b-0"><span className="grid size-8 place-items-center rounded-full bg-muted text-xs font-bold">{user.initials}</span><span className="min-w-0 flex-1"><b className="block text-xs">{user.name}</b><small className="text-xs text-muted-foreground">{user.email}</small></span><span className="hidden text-xs sm:block">{user.department}</span>{user.governanceAssignments.length > 0 && <span className="rounded-md bg-primary/10 px-2 py-1 text-xs font-semibold text-primary">{user.governanceAssignments.length} governance</span>}<span className={`rounded-full px-2 py-1 text-xs ${user.status === "Active" ? "bg-[var(--dv-badge-ok-bg)] text-[var(--dv-badge-ok-text)]" : "bg-muted text-muted-foreground"}`}>{user.status}</span></div>)}{!users.length && <p className="p-6 text-center text-xs text-muted-foreground">No users assigned.</p>}</div></section>;
}
