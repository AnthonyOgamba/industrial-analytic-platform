...
export function RegisterSiteModal({ managers, governancePolicies=[], onClose, onSave }: { managers: SiteManagerOption[]; governancePolicies?:string[]; onClose: () => void; onSave: (value:RegisteredFacility,existingFacilityId?:number) => Promise<RegistrationResult> }) {
  ...
  const [committedFacilityId,setCommittedFacilityId]=useState<number|null>(null);
  ...
  async function submit(event: FormEvent) {
    ...
    try{const result=await onSave({settings:{...settings,code:safeCode,managerUserId:String(manager.uid)},facility:{ id: `facility-${safeCode.toLowerCase()}-${Date.now()}`, name: settings.name.trim(), code: safeCode, facilityType: "Plant", company: "", managerId: String(manager.uid), manager: manager.username, source: "registered", status, location: { city: settings.city.trim(), region: settings.provinceState.trim(), country: settings.country.trim() }, timezone: settings.timezone, shiftPattern: "", capacityPerDay: "", sensorCount: 0, assetCount: 0, complianceScore: 0, lastActivity: "", halls }},committedFacilityId??undefined);if(result.hierarchyComplete){onClose();return}setCommittedFacilityId(result.facilityId);setError(`Facility created successfully, but hierarchy setup is incomplete.${result.failedEndpoint?` Failed endpoint: ${result.failedEndpoint}.`:""}${result.failureMessage?` ${result.failureMessage}`:""}`);}catch(cause){if(cause instanceof ApiError&&Object.keys(cause.fieldErrors).length){const mapped=Object.fromEntries(Object.entries(cause.fieldErrors).filter(([key])=>key in emptyFacilitySettings)) as FacilityFieldErrors;setFieldErrors(mapped);const first=Object.keys(mapped)[0];if(first)setTimeout(()=>document.querySelector<HTMLElement>(`[name="${first}"]`)?.focus(),0);if(Object.keys(cause.fieldErrors).some(key=>!(key in emptyFacilitySettings)))setError(cause.message)}else setError(cause instanceof Error?cause.message:"Facility registration failed.")}finally{setPending(false)}
  }

  return <ModalFrame title={committedFacilityId?"Completing hierarchy":"Register Manufacturing Site"} description="Create the facility, hierarchy, manager assignment, governance, and financial settings." icon={<Factory className="size-5" />} onClose={onClose}><form onSubmit={submit} className="space-y-4 p-5"><FacilitySettingsFields form={settings} change={change} managers={managers} policies={governancePolicies} errors={fieldErrors}/>{error&&<p role="alert" className="rounded-lg bg-destructive/10 p-3 text-xs text-destructive">{error}</p>}<fieldset className="rounded-xl border p-4"><legend className="font-mono text-xs font-semibold uppercase text-primary">Factory hierarchy</legend><div className="grid gap-3 sm:grid-cols-3"><label className={labelClass}>Halls<input type="number" min={1} max={20} value={hallCount} onChange={e=>setHallCount(Number(e.target.value))} className={inputClass}/></label><label className={labelClass}>Lines per hall<input type="number" min={1} max={20} value={linesPerHall} onChange={e=>setLinesPerHall(Number(e.target.value))} className={inputClass}/></label><label className={labelClass}>Stations per line<input type="number" min={1} max={50} value={stationsPerLine} onChange={e=>setStationsPerLine(Number(e.target.value))} className={inputClass}/></label></div></fieldset><footer className="flex justify-end gap-2 border-t pt-4"><button type="button" onClick={onClose} className="h-10 rounded-lg border px-4">Cancel</button><button type="submit" disabled={!managers.length||!governancePolicies.includes(settings.governancePolicy)||pending} className="h-10 rounded-lg bg-primary px-4 text-primary-foreground disabled:opacity-40">{pending?(committedFacilityId?"Continuing…":"Registering…"):(committedFacilityId?"Continue setup":"Register Site")}</button></footer></form></ModalFrame>;
}
