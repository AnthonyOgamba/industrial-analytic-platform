...
export function FacilitiesWorkspace() {
  ...
  async function registerFacility({facility,settings}: RegisteredFacility,existingFacilityId?:number):Promise<RegistrationResult> {
    ...
    try{
      ...
      if(!workspace.siteAccess.some(item=>item.facilityId===facilityId&&item.userId===Number(facility.managerId))){endpoint="/api/backend/site-access";await apiRequest(endpoint,{method:"POST",body:JSON.stringify({userId:Number(facility.managerId),facilityId,accessLevel:"manager"})})}
      ...
      const verified=await invalidateFacilityHierarchy();const completed=verified.facilities.find(item=>item.facilityId===facilityId);if(!completed||completed.halls.length<facility.halls.length)throw new Error("The refreshed workspace did not return the completed hierarchy.");await load();setError("");return{facilityId,hierarchyComplete:true};
    }catch(cause){await load();return{facilityId,hierarchyComplete:false,failedEndpoint:endpoint,failureMessage:cause instanceof ApiError?`${cause.status}: ${cause.message}`:cause instanceof Error?cause.message:"Hierarchy setup failed."}}
  }
