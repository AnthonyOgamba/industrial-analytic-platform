...
export function FacilitiesWorkspace() {
  ...
  async function retireFacility(facility:Facility){if(!canManage||!confirm(`Delete ${facility.name}? The backend will retire this facility and preserve its audit history. This can be blocked while active runs or dependent resources exist.`))return;setError("");try{await facilityHierarchyApi.updateFacility(Number(facility.id),{name:facility.name,code:facility.code||null,status:"retired"});await load()}catch(cause){setError(cause instanceof Error?cause.message:"The facility could not be retired.")}}
