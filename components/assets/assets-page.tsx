...
export function AssetsPage() {
  ...
  const [financial,setFinancial] = useState<FinancialSnapshotDto[]>([]);
  ...
  const canCreate=session.user?.capabilities.includes("assets.manage")??false;
  ...
  const loadRelated = useCallback(async()=>{
    ...
    const results = await Promise.allSettled([
      ...
      apiRequest<PagedEnvelope<FinancialSnapshotDto>>("/api/backend/financial?page=1&pageSize=500&includeSynthetic=true"),
  ...
  const assets=useMemo(()=>canonicalAssets.map(record=>{for(const facility of hierarchy.data?.facilities??[])for(const hall of facility.halls)for(const line of hall.lines){const station=line.stations.find(item=>item.stationId===record.station_id);if(station)return{assetId:record.asset_id,stationId:station.stationId,name:record.asset_name,code:station.code,status:record.status,machineType:record.machine_type,firmware:record.firmware_version,criticality:record.criticality,facilityId:facility.facilityId,facility:facility.name,hallId:hall.hallId,hall:hall.name,lineId:line.productionLineId,line:line.name,oee:station.performance.oee,availability:station.performance.availability,performance:station.performance.performance,quality:station.performance.quality}}return{assetId:record.asset_id,stationId:record.station_id,name:record.asset_name,code:null,status:record.status,machineType:record.machine_type,firmware:record.firmware_version,criticality:record.criticality,facilityId:record.facilityid??0,facility:record.facility_name,hallId:record.hallid??0,hall:record.hall_name,lineId:record.productionlineid??0,line:record.line_name,oee:0,availability:0,performance:0,quality:0}}),[canonicalAssets,hierarchy.data]);
  ...
  const financeFor=(stationId:number)=>financial.filter(item=>item.stationId===stationId);
  ...
  return <div className="space-y-5 pb-5">
    <header className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div><p className="font-mono text-[10px] font-semibold uppercase tracking-[0.14em] text-primary">Operations</p><h1 className="mt-1.5 text-2xl font-bold tracking-tight">Asset Registry</h1><p className="mt-1 text-sm text-muted-foreground">Backend stations connected to hierarchy, sensors, Olive risk, downtime, and financial impact</p></div><div className="flex gap-2">{canCreate&&<button onClick={()=>setCreateOpen(true)} className="inline-flex h-10 items-center gap-2 rounded-lg bg-primary px-4 text-xs font-semibold text-primary-foreground"><Plus className="size-4"/>Create Asset</button>}<button onClick={()=>void Promise.all([hierarchy.refresh(),loadRelated()])} className="inline-flex h-10 items-center gap-2 rounded-lg border px-4 text-xs"><RefreshCw className="size-4"/>Refresh</button></div></header>
    ...
    {loading?<div role="status" className="h-64 animate-pulse rounded-xl bg-muted"/>:<div className="overflow-hidden rounded-xl border bg-card"><div className="overflow-x-auto"><table className="w-full min-w-[78rem] text-left text-xs"><thead className="bg-muted/40 font-mono text-[9px] uppercase text-muted-foreground"><tr>{["Asset / Code","Hierarchy","Status","Sensors","OEE","AI health","Failure risk","Predicted maintenance","Active downtime","Last telemetry"].map(label=><th key={label} className="px-3 py-3">{label}</th>)}</tr></thead><tbody>{visible.map(item=>{const prediction=riskFor(item.stationId);const sensorCount=sensorsFor(item).length;const activeDowntime=downtimeFor(item.stationId).filter(event=>!event.endTime).length;return <tr key={item.stationId} className="cursor-pointer border-t hover:bg-muted/30" onClick={()=>setSelected(item)}><td className="px-3 py-3"><strong>{item.name}</strong><p className="font-mono text-[9px] text-muted-foreground">{item.code??`Station ${item.stationId}`}</p></td><td className="px-3 py-3">{item.facility}<p className="text-[10px] text-muted-foreground">{item.hall} · {item.line}</p></td><td className="px-3 py-3 uppercase">{item.status}</td><td className="px-3 py-3">{sensorCount}</td><td className="px-3 py-3">{item.oee.toFixed(1)}%</td><td className="px-3 py-3">{prediction?`${Math.max(0,100-prediction.failure_probability*100).toFixed(0)}%`:"Unavailable"}</td><td className="px-3 py-3 uppercase">{riskLabel(prediction)}</td><td className="px-3 py-3 text-muted-foreground">Not supplied</td><td className="px-3 py-3">{activeDowntime}</td><td className="px-3 py-3 text-muted-foreground">Not supplied</td></tr>})}{!visible.length&&<tr><td colSpan={10} className="p-12 text-center text-muted-foreground">{assets.length?"No station assets match the current filters.":"No station assets exist in the accessible facility hierarchy."}</td></tr>}</tbody></table></div></div>}
    {selected&&<div className="fixed inset-0 z-50 bg-black/50" onMouseDown={event=>event.target===event.currentTarget&&setSelected(null)}><aside className="ml-auto h-full w-[min(42rem,96vw)] overflow-y-auto border-l bg-background p-5 shadow-2xl"><header className="flex items-start gap-3 border-b pb-4"><Cpu className="mt-1 size-5 text-primary"/><div className="flex-1"><h2 className="text-lg font-bold">{selected.name}</h2><p className="font-mono text-[10px] text-muted-foreground">{selected.code??`Station ${selected.stationId}`}</p></div><button onClick={()=>setSelected(null)} className="grid size-9 place-items-center rounded-lg border"><X className="size-4"/></button></header><div className="space-y-4 py-5"><section className="rounded-xl border bg-card p-4"><h3 className="text-xs font-bold">Hierarchy</h3><p className="mt-2 text-xs">{selected.facility} → {selected.hall} → {selected.line} → {selected.name}</p><div className="mt-3 grid grid-cols-4 gap-2 text-center">{[["OEE",selected.oee],["Availability",selected.availability],["Performance",selected.performance],["Quality",selected.quality]].map(([label,value])=><div key={String(label)} className="rounded-lg bg-muted/40 p-2"><strong>{Number(value).toFixed(1)}%</strong><p className="text-[8px] text-muted-foreground">{label}</p></div>)}</div></section><section className="rounded-xl border bg-card p-4"><h3 className="text-xs font-bold">Connected sensors</h3>{sensorsFor(selected).length?<div className="mt-2 space-y-2">{sensorsFor(selected).map(item=><div key={item.sid} className="flex justify-between rounded-lg bg-muted/40 p-2 text-xs"><span>{item.name} · {item.sensorType}</span><span>{item.status}</span></div>)}</div>:<p className="mt-2 text-xs text-muted-foreground">No backend sensor stream is assigned to this station.</p>}</section><section className="rounded-xl border bg-card p-4"><h3 className="flex items-center gap-2 text-xs font-bold"><Bot className="size-4 text-primary"/>Olive AI risk</h3>{riskFor(selected.stationId)?<div className="mt-2 text-xs"><p className="uppercase">{riskFor(selected.stationId)?.risk_level} · {(riskFor(selected.stationId)!.failure_probability*100).toFixed(1)}% failure probability</p><p className="mt-2 text-muted-foreground">{riskFor(selected.stationId)?.recommendation}</p><p className="mt-2 font-mono text-[9px]">Calculated {new Date(riskFor(selected.stationId)!.calculated_at).toLocaleString()}</p></div>:<p className="mt-2 text-xs text-muted-foreground">No AI risk prediction exists for this station.</p>}</section><section className="grid gap-3 sm:grid-cols-3">{[{label:"Open alerts",value:alertsFor(selected).filter(item=>item.status==="open").length},{label:"Downtime events",value:downtimeFor(selected.stationId).length},{label:"Financial records",value:financeFor(selected.stationId).length}].map(item=><div key={item.label} className="rounded-xl border bg-card p-3"><strong>{item.value}</strong><p className="text-[9px] text-muted-foreground">{item.label}</p></div>)}</section><div className="flex flex-wrap gap-2"><Link href={`/sensors?stationId=${selected.stationId}`} className="rounded-lg border px-3 py-2 text-xs">Connected Sensors</Link><Link href={`/downtime?stationId=${selected.stationId}`} className="rounded-lg border px-3 py-2 text-xs">Downtime History</Link><Link href={`/financial?stationId=${selected.stationId}`} className="rounded-lg border px-3 py-2 text-xs">Financial Impact</Link><Link href={`/operations?stationId=${selected.stationId}`} className="rounded-lg border px-3 py-2 text-xs">Hierarchy View</Link></div></div></aside></div>}
    {createOpen&&hierarchy.data&&<CanonicalAssetForm hierarchy={hierarchy.data} onClose={()=>setCreateOpen(false)} onCreated={async()=>{await loadRelated();setSuccess("Asset created successfully.")}}/>}
  </div>;
}
