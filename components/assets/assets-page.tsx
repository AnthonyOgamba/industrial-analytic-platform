...
export function AssetsPage() {
  ...
  const riskFor=(stationId:number)=>safeRisks.find(item=>item.station_id===stationId);
  ...
  return <div className="space-y-5 pb-5">
    ...
    {createOpen&&hierarchy.data&&<CanonicalAssetForm hierarchy={{...hierarchy.data,facilities:accessibleFacilities}} onClose={()=>setCreateOpen(false)} onCreated={async()=>{await Promise.all([hierarchy.refresh(),loadAssets()]);setSuccess("Asset created successfully.")}}/>}
  </div>;
}
