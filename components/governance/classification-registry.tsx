...
function DetailRows({ rows }: { rows: string[][] }) {
  return <dl>{rows.map(([label, value]) => <div key={label} className="grid gap-1 border-b py-3 last:border-b-0 sm:grid-cols-[12rem_1fr] sm:items-center"><dt className="font-mono text-xs font-medium uppercase tracking-[0.14em] text-muted-foreground">{label}</dt><dd className="text-sm font-semibold">{value}</dd></div>)}</dl>;
}
