...
export function AuditEventRow({
  ...
  return (
    <button
      ...
      className="grid w-full gap-2 border-t px-3 py-3 text-left transition-colors first:border-t-0 hover:bg-muted/40 sm:grid-cols-[minmax(0,1fr)_auto] sm:px-4"
    >
      ...
      <div className="flex items-center justify-between gap-3 pl-10 sm:block sm:min-w-24 sm:pl-0 sm:text-right">
        ...
        <div className="mt-1 text-[9px] text-muted-foreground">
          ...
          <span className="ml-3 font-mono sm:ml-0 sm:block">{event.ip}</span>
        </div>
      </div>
    </button>
  );
...
export function AuditEventDetailModal({
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex items-end justify-center bg-black/65 backdrop-blur-sm sm:items-center sm:p-5"
      ...
      >
        <header className="flex items-start gap-3 border-b p-5">
          ...
          <div className="min-w-0 flex-1">
            <h2 id="audit-event-title" className="font-bold">
              {friendlyAuditLabel(event.action)} —{" "}
              {friendlyAuditLabel(event.resource)}
            </h2>
        ...
        <div className="space-y-4 p-5">
          <div className="flex gap-2">
            <AuditStatusBadge status={event.status} />
            <AuditResourceBadge resource={event.resource} />
          </div>
          <p className="rounded-xl border bg-muted/40 p-4 text-xs leading-5">
            {event.detail}
          </p>
          ...
          <button
            type="button"
            onClick={onClose}
            className="h-9 w-full rounded-lg border text-xs font-semibold hover:bg-muted"
          >
            Close
          </button>
        </div>
