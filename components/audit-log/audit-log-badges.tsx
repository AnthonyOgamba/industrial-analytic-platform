...
export function AuditStatusBadge({ status }: { status: AuditStatus }) {
  ...
  return <span className={`inline-flex items-center gap-1 rounded-full px-2 py-1 text-xs font-semibold ${styles[status]}`}><Icon className="size-3" />{status}</span>;
}

export function AuditActionBadge({ action }: { action: string }) { return <span className="font-mono text-xs font-semibold text-foreground">{action}</span>; }
...
export function AuditResourceBadge({ resource }: { resource: string }) { return <span className={`rounded px-1.5 py-0.5 font-mono text-xs font-semibold ${colors[resource] ?? colors.Settings}`}>{resource}</span>; }
