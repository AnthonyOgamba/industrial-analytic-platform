...
export function DataQualityPanel() {
  return <aside className="rounded-xl border bg-card p-5 shadow-[var(--dv-shadow)]"><div className="flex items-center justify-between"><h2 className="text-sm font-semibold">Data Quality</h2><span className="rounded-full bg-[var(--dv-badge-ok-bg)] px-2 py-1 font-mono text-xs text-[var(--dv-badge-ok-text)]">Governance ready</span></div><div className="mt-5 space-y-4">{dataQualityMetrics.map((metric) => <DataQualityMetric key={metric.label} {...metric} />)}</div><p className="mt-5 border-t pt-4 text-xs leading-5 text-muted-foreground">Validation, classification, encryption, and retention metadata are prepared before governance handoff.</p></aside>;
}
