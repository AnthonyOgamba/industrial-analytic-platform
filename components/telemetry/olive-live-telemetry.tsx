  ...
  return <section className="space-y-3">
    <div className="flex flex-col gap-3 rounded-xl border bg-card p-4 sm:flex-row sm:items-center sm:justify-between">
      ...
      <div className="flex flex-wrap items-center gap-2">
        <span className="rounded-full bg-muted px-2.5 py-1 font-mono text-[9px] uppercase">{events.status}</span>
        <span className="text-[10px] text-muted-foreground">Last event: {events.lastEventAt ? new Date(events.lastEventAt).toLocaleString() : "Waiting"}</span>
    ...
    <p className="rounded-lg border bg-muted/30 p-3 text-[10px] text-muted-foreground">{events.replayNotice}</p>
    <div className="overflow-x-auto rounded-xl border bg-card">
      <table className={`w-full text-left text-xs ${compact ? "min-w-[72rem]" : "min-w-[90rem]"}`}>
        ...
        <thead className="border-b bg-muted/50 font-mono text-[9px] uppercase text-muted-foreground"><tr>{["Time","Facility","Hall","Line","Station","Sensor","Type","Value","Status","Threshold","Anomaly","Run","Source"].map(label=><th scope="col" key={label} className="p-3 font-medium">{label}</th>)}</tr></thead>
        <tbody>{visible.map(reading=><tr key={`${reading.event_id ?? reading.reading_id}-${reading.reading_id}`} className="border-b last:border-0">
          <td className="p-3 font-mono text-[9px]">{new Date(reading.reading_ts).toLocaleString()}</td><td className="p-3">{reading.facility_name}</td><td className="p-3">{reading.hall_name}</td><td className="p-3">{reading.production_line_name}</td><td className="p-3"><strong>{reading.station_name}</strong><p className="font-mono text-[9px] text-muted-foreground">{reading.station_code}</p></td><td className="p-3">{reading.sensor_name}</td><td className="p-3">{reading.sensor_type}</td><td className="p-3 font-mono font-bold">{reading.value} {reading.unit ?? "Unit unavailable"}</td><td className="p-3">{reading.status}</td><td className="p-3">{reading.threshold_condition ?? "None"}</td><td className="p-3">{reading.is_anomaly ? "Anomaly" : "Normal"}</td><td className="p-3 font-mono">#{reading.run_id}</td><td className="p-3">{reading.source}{reading.is_synthetic ? " · synthetic" : ""}</td>
