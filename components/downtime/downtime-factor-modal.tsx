...
function Section({ title, children }: { title: string; children: ReactNode }) { return <fieldset className="space-y-3 border-t pt-5 first:border-0 first:pt-0"><legend className="mb-3 font-mono text-xs font-semibold uppercase tracking-[0.12em] text-primary">{title}</legend>{children}</fieldset>; }
function CheckboxGroup({ label, options, selected, onChange, scrollable = false }: { label: string; options: { value: string; label: string }[]; selected: string[]; onChange: (values: string[]) => void; scrollable?: boolean }) { return <div><p className="mb-2 text-xs font-medium">{label}</p><div className={`${scrollable ? "max-h-28" : "max-h-36"} overflow-y-auto rounded-lg border bg-background p-2`}><div className="flex flex-wrap gap-2">{options.map((option) => { const checked = selected.includes(option.value); return <label key={option.value} className={`flex cursor-pointer items-center gap-1.5 rounded-md border px-2 py-1.5 text-xs ${checked ? "border-primary/40 bg-primary/10 text-primary" : "text-muted-foreground"}`}><input type="checkbox" checked={checked} onChange={(event) => onChange(event.target.checked ? [...selected, option.value] : selected.filter((value) => value !== option.value))} className="size-3 accent-[var(--primary)]" />{option.label}</label>; })}{!options.length && <span className="px-1 py-2 text-xs text-muted-foreground">No matching options</span>}</div></div></div>; }
