...
import { aiAgentUrl, expireAuthentication, gatewayFailure } from "@/lib/backend-api";
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const upstream = await fetch(aiAgentUrl("/api/ai/events"), { headers, cache: "no-store", signal: request.signal });
