...
export type FacilityManagerOption={uid:number;username:string;email:string;role:string};
export type FacilitiesPageContract=PageStatus&{facilities:FacilityWorkspace;risk:AiFailureProbability[];managerOptions:FacilityManagerOption[]};
export type ApiGatewayPageContract=PageStatus&{status:"available";service:{status:"available";environment:string|null;authenticationStatus:string;rateLimitStatus:string|null};routing:{summary:string};recentRequests:{total:number|null;successful:number|null;failed:number|null}};
