...
assert.match(workspace,/apiRequest\("\/api\/backend\/facilities\/register",\{method:"POST"/,"registration uses the transactional endpoint");
assert.doesNotMatch(workspace,/apiRequest<unknown>\("\/api\/backend\/facilities",\{method:"POST"/,"registration no longer creates the facility separately");
assert.doesNotMatch(workspace,/for\(const\[hallIndex,hall\]of facility\.halls\.entries\(\)\)/,"registration no longer orchestrates child creation");
assert.match(workspace,/await invalidateFacilityHierarchy\(\)/,"successful registration refreshes canonical hierarchy");
assert.match(modal,/hierarchy:\{hallCount,linesPerHall,stationsPerLine\}/,"one request contains hierarchy counts");
assert.doesNotMatch(modal,/committedFacilityId|Continue setup/,"atomic registration has no partial-success state");
